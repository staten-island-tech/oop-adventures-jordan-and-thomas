...
class functionality():
    ...
    def damagecalc(damagecalc, move, attackingpk, enemypk):
        ...
        if moves[movenumber]["category"] == "None":
            movedamage = 0
    def specialeffect(specialeffect, move, damage, enemyspeed, enemypk, userpk, enemypartystatus, enemymove, enemydamage, movego, userpartyhealth):
        f = functionality
        global damageeffect
        damageeffect = False
        global newdamage
            ...
            for i in range(len(enemypartystatus)):
                if enemypk == enemypartystatus[i - 1]:
                    if not("Electric" in data[enemynumber]["Types"]):
                        enemypartystatus[i] = "Paralyzed"
                        print(enemypokemon, "got paralyzed!")
        if "Flinch" in moves[movenumber]["effect"]:
            Turns.speedcheck(enemyspeed, currentspeed)
    
            flinch = random.randrange(10)
            if "Enemy" in goingfirst:
                if flinch == 1:
                    print(currentpokemon, "flinched and was unable to move")
                    youflinch = "yes"
            if "User" in goingfirst:
                if flinch == 1:
                    print(enemypokemon, "flinched and was unable to move")
                    enemyflinch = "no"
            ...
            if userdo == "Switch" or userdo == "switch" or userdo == "Switch Out" or userdo == "switch out" or userdo == "Switch out":
                ...
                if gomove == True:
                    ...
                    if paralyzed == False:
                        ...
                        thing = f.specialeffect(enemymove, enemydamage, currentspeed, currentpokemon, enemypokemon, userpartystatus, "Captain Underpants' Theme Song", "A and W", gomove, enemypartyhealth)
                        if newdamage == True:
                            enemydamage = thing
                        if newspeed == True:
                            enemyspeed = thing
                        ...
                        if currentmoves[i] == use:
                            if Weezer == True:
                                ...
                                if paralyzed == False:
                                    ...
                                    damage = movedamage
                                    thing = f.specialeffect(use, damage, enemyspeed, enemypokemon, currentpokemon, eliteteamstatus, "Heroes", "Christmas Wrapping", Weezer, userpartyhealth)
                                    if newdamage == True:
                                        damage = thing
                                    if newspeed == True:
                                        enemyspeed = thing
...
<<<<<<< HEAD
                    ...
                    if gomove == True and flinched == False:
                        ...
                        if paralyzed == False:
                            ...
                            enemydamage = movedamage
                            thing = f.specialeffect(enemymove, enemydamage, currentspeed, currentpokemon, enemypokemon, userpartystatus, use, damage, gomove, enemypartyhealth)
                            if newdamage == True:
                                enemydamage = thing
                            if newspeed == True:
                                currentspeed = thing
...
=======
                    ...
                    if enemyflinch == "no":                
                        ...
                        gomove = f.accuracycheck(enemymove)
                        if gomove == True:
>>>>>>> 6d51e0e56ea041101cffcdde3a9e3b9cf0990bac
                            for i in range(len(userpartystatus)):
                                if userpartystatus[i] == "Paralyzed":
                                    paralyzechance = 25
                                    pchance = random.randint(0, 100)
                ...
                if "Enemy" in goingfirst:
                    ...
                    if gomove == True:
                        ...
                        if paralyzed == False:
                            f.damagecalc(enemymove, enemypokemon, currentpokemon)
                            enemydamage = movedamage
                            thing = f.specialeffect(enemymove, enemydamage, currentspeed, currentpokemon, enemypokemon, userpartystatus, "Barry Bonds", "LOVE.FEAT.ZACARI", gomove, enemypartyhealth)
                            if newdamage == True:
                                enemydamage = thing
                            if newspeed == True:
                                currentspeed = thing
                    ...
                    if death == False and youflinch == "no":
                        time.sleep(times)
                        going = "You"
                        print("You used", use)
                        time.sleep(times)
                            ...
                            if currentmoves[i] == use:
                                ...
                                if Weezer == True and flinched == False:
                                    ...
                                    if paralyzed == False:
                                        f.damagecalc(use, currentpokemon, enemypokemon1)
                                        damage = movedamage
                                        thing = f.specialeffect(use, damage, enemyspeed, enemypokemon, currentpokemon, eliteteamstatus, enemymove, enemydamage, Weezer, userpartyhealth)
                                        if newdamage == True:
                                            damage = thing
                                        if newspeed == True:
                                            enemyspeed = thing
